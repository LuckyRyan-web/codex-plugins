...
用户要求先评审当前阶段，或后续动作需要选择、批准、授权时，Agent 应提交
`status: "needs_user"` 并停下等待。这个状态不要求先通过整个任务的完成检查
或独立验收。最终回复明确表示当前等待用户决定时，Hook 也会转入 `waiting_user`，
即使完成声明缺失或误写为 `complete`，也不会因此催促越过确认节点。

等待不是完成。重复 Stop 和旧 Hook 自动续跑不会唤醒等待；真实用户回复后
才重新处理，且不把用户的普通追问当成批准。引用、示例、条件句或仅提到确认
不触发这项兜底。未知的工具验证结果仍保持未知，不会因为等待而被记作成功。
