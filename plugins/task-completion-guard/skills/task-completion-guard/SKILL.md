...
用户只是就已有工作提问时，直接回答即可：任务、验收项和已提交的声明都不变，
这一轮不需要重新提交完成审计。带动作诉求的说法（例如“能不能改成 X”）不是
提问，按新要求处理。
...
- `routine`：仅供细小低风险的文案或格式调整。不超过 1 个文件、不超过 20 行
  且不含任何代码或配置文件才可免除；改动一旦涉及代码，该标签不再免除验收。
...
把 fail-open 当作通过。验收管线本身失败——`prepare-review` 因快照范围超出
限制而无法准备、子 Agent 起不来、身份始终无法绑定、或已用尽验收次数——
用 `status: "blocked"` 加具体 `reason` 收尾，不要反复重交 `complete`。快
照范围包含工作区所有改动与未跟踪文件，超限时改换参数重跑同样会失败。
